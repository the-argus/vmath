...
VMATH_INLINE vm_v2f_t vm_mulc_v2f(const vm_v2f_t a, const vm_float32_t constant)
{
#if defined(VMATH_SSE41_ENABLE)
	// clang-format off
    #ifdef VMATH_USE_DISTINCT_TYPE_V2
        vm_v2f_t out;
        out.inner = _mm_mul_ps(a.inner, vm_splat_v2f(constant).inner);
        return out;
    #else
        return _mm_mul_ps(a, vm_splat_v2f(constant));
    #endif
	// clang-format on
