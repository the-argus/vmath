...
VMATH_INLINE vm_8batch_v2f_t vm_div_8xv2f_constant(const vm_8batch_v2f_t a,
												   const float b)
{
#define VMATH_DIV_8XV2_CONSTANT_SCALAR()                                       \
	...
	return _mm512_div_ps(a, vm_splat_8xv2f(b));
#else
	vm_8batch_v2f_t result;
	const vm_2batch_v2f_t constants = vm_splat_2xv2f(b);
#pragma unroll
	for (int8_t i = 0; i < 4; ++i) {
		result.buffer[i] = _mm_div_ps(a.buffer[i], constants);
	}
	return result;
#endif
#else
	VMATH_DIV_8XV2_CONSTANT_SCALAR()
...
#error ARM SIMD not implemented
	...
	VMATH_DIV_8XV2_CONSTANT_SCALAR()
#endif
#undef VMATH_DIV_8XV2_CONSTANT_SCALAR
}
